...
def sendRequest(url, year, page_index, token, cookie, province, retry_times):
    ...
    headers = {
        ...
        "Referer": "http://223.220.143.64:2021/pub/GongShiSearch",
...
try:
    ...
    for province in provinces:
        if province['name'] == '青海省':
            url = province['url']
            province_name = province['name']
            get_province_details_by_requests(url, province_name, '2021', '1', '392')
            get_province_details_by_requests(url, province_name, '2022', '1', '502')
            get_province_details_by_requests(url, province_name, '2023', '1', '612')
            get_province_details_by_requests(url, province_name, '2024', '1', '518')
